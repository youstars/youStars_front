...
const ModalOrders: React.FC<ModalOrdersProps> = ({closeModal}) => {
    ...
    const handleSubmit = async () => {
        ...
        const start_project_at = dateFrom ? dateFrom.toISOString() : null;
        ...
        // данные для валидации (нужен userRole, но на сервер не отправляем)
        const validationData = { ...orderData, userRole };

        try {
            await orderSchema.validate(validationData, {abortEarly: false});
            setErrors({});
                ...
                <div className={styles.formContainer}>
                    ...
                    <div className={styles.budgetDateRow}>
                        ...
                        <div className={styles.dateGroup}>
                            ...
                            {formTouched && (errors.start_project_at || errors.project_deadline) && (
                                <p className={styles.errorText}>{errors.start_project_at || errors.project_deadline}</p>
                            )}
                        </div>
                    </div>
                    {userRole !== "Client" && (
