...
import classes from "./SpecialistProject.module.scss";
...
const specialist_id = getCookie("user_role_id");
...
export default function SpecialistProject({ project }: { project: Project }) {
  ...
  const trackerName = project.tracker?.custom_user?.full_name ?? "—";
  ...
  return (
    <div
      ...
      onClick={() =>
        navigate(`/manager/specialist/${specialist_id}/project/${project.id}`)
      }
    >
      <div className={classes.top}>
        <div className={classes.orderBox}>
          ...
          >{`Заявка №${project.order} / ${project.name}`}</div>
          <div className={classes.gene}>
            <div className={classes.budget}>
              {project.budget != null
                ? `${Math.round(Number(project.budget)).toLocaleString(
                    "ru-RU"
                  )} ₽`
                : "—"}
            </div>
        ...
        <div className={classes.icons} onClick={(e) => e.stopPropagation()}>
          <button
            ...
            type="button"
            onClick={() =>
              navigate(`/specialist/project/${project.id}/messages`)
            }
          >
          ...
          {project.specialists && project.specialists.length > 0
            ? project.specialists.map((s, idx) => (
                <React.Fragment key={s.id}>
                  <span className={classes.specialistItem}>
