...
const SpecialistCard: React.FC<{ specialistId: number; isSelf?: boolean }> = ({
  specialistId,
  isSelf = false,
}) => {
  ...
  const activeProjects = specialistProjects.filter(
    (project: ProjectDetail) => project.status === "in_progress"
  );
  const finishedProjects = specialistProjects.filter(
    (project: ProjectDetail) => project.status === "completed"
  );
  ...
  return (
    <div className={styles.main}>
      <div className={styles.container}>
        ...
        />
        <FilesAndTags
          files={specialist.file?.map(
            (f: { id: number; name: string; file: string }) => ({
              id: f.id,
              name: f.name,
              fileUrl: f.file,
            })
          ...
          tags={
            specialist.professional_profiles
              ?.map(
                (p: { profession?: { name?: string } }) => p.profession?.name
              )
