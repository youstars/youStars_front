...
export default function BusinessApplicationCard({ order }: Props) {
  ...
  const currentStepIndex = steps.findIndex(
    (step) => step === statusStepsMap[order.status as keyof typeof statusStepsMap]
  );
...
const handleFieldBlur = (
    ...
    <div className={styles.container}>
        ...
        <div className={styles.rightBlock}>
          <div className={styles.avatar}>
            <Avatar size="40" />
            <h1>{order.project_name || order.order_name}</h1>
          </div>
  ...
  onBlur={() => handleFieldBlur("order_goal", order.order_goal || "", goal)}
/>
...
<TextAreaField
  ...
  onBlur={() =>
    handleFieldBlur("solving_problems", order.solving_problems || "", problems)
  }
/>
...
<TextAreaField
  ...
  onBlur={() =>
    handleFieldBlur("product_or_service", order.product_or_service || "", product)
  }
/>
